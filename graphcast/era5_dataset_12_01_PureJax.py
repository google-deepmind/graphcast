...
def data_merge_make_circular_one_hot_varradius_cpu(
    ...
    height: int,
    width: int,
    centers_with_radii: list,  # List of (ci, cj, r) for multiple storms
) -> torch.Tensor:
    """
    Generate one-hot tensor with multiple circular regions, each with its own radius.
    """
    yy = np.arange(height)[:, None]
    xx = np.arange(width)[None, :]

    mask2d = np.zeros((height, width), dtype=np.int64)
    for ci, cj, rr in centers_with_radii:
        # rr = max(1, int(round(rr))) # 安全：半径至少1，且转成int
        # 半径至少 1，小数部分 > 0.3 就进位，且转成 int
        frac = rr - int(rr)
        if frac > 0.3:
            rr = int(np.ceil(rr))   # 小数部分 >0.3 → 进位
        else:
            rr = int(np.floor(rr))  # 否则舍去小数
        rr = max(1, rr)             # 半径至少为 1
        curr = ((yy - ci)**2 + (xx - cj)**2 <= rr**2)
        mask2d |= curr
...
class DateMergedERA5TyphoonSizeDataset(Dataset):
    def __init__(
        self,
        full_ds: xr.Dataset,
        task_config,
        ...
        end_year: int,
        label_radius: int = 5,
        month_filter: list[int] | None = None,   # 新增：按月份白名单过滤，例如 [8,9,10]
        day_filter: list[int] | None = None,     # 新增：按日期（1-31）白名单过滤
        date_filter: list[str] | None = None,    # 新增：按具体日期白名单过滤，例如 ["2016-10-03","2016-10-04"]
    ):
        """
        ...
        # 1) Read the single CSV (tracks_folder 可以是文件或目录)
        if os.path.isdir(tracks_folder):
            # 目录里找唯一的 .csv
            csv_candidates = [os.path.join(tracks_folder, f) for f in os.listdir(tracks_folder) if f.endswith(".csv")]
            assert len(csv_candidates) >= 1, f"No .csv found under {tracks_folder}"
            csv_path = csv_candidates[0]
        else:
            # 直接就是文件路径
            csv_path = tracks_folder
        ...
        # ====== 新增：可选的月份/日期/具体日期过滤（默认 None 不生效） ======
        # month_filter = [9, 10]      # 或设为 None
        # day_filter   = None         # e.g., [1,2,3]；或 None
        # date_filter  = None         # e.g., ["2018-09-12","2018-10-04"]；或 None

        if month_filter is not None and len(month_filter) > 0:
            if "month" not in df.columns:
                raise ValueError("CSV must contain a 'month' column for month_filter to work.")
            df = df[df["month"].isin(month_filter)]

        if day_filter is not None and len(day_filter) > 0:
            if "day" not in df.columns:
                raise ValueError("CSV must contain a 'day' column for day_filter to work.")
        ...
        if date_filter is not None and len(date_filter) > 0:
            ...
            df = df[datestr.isin(set(date_filter))]
        # ============================================================

        # 3) 只保留必要列；rsize 是 1° 网格单位（你已确认）
        needed = ["track_id", "year", "month", "day", "hour", "lon", "lat"]
        if "rsize" in df.columns:
            needed.append("rsize")
        elif "rsize_km" in df.columns:
            # 兜底：如果只有 km，粗略转成度（≈ /111）；你也确认我们用 rsize（1°），这里只做防守
            df["rsize"] = df["rsize_km"] / 111.0
            needed.append("rsize")
        df = df[needed].copy()

        # 4) 类型清理（避免字符串类型混入）
        for c in ["track_id", "year", "month", "day", "hour"]:
            df[c] = df[c].astype(int)
        for c in ["lon", "lat", "rsize"]:
            if c in df.columns:
                df[c] = pd.to_numeric(df[c], errors="coerce")

        # 4.1) 构造时间戳列（对齐到小时）
        df["ts"] = pd.to_datetime(dict(
            year=df["year"], month=df["month"], day=df["day"], hour=df["hour"]
        ))

        # 5) 与 ERA5 时间轴对齐（只保留 ERA5 存在的时刻）
        valid_ts = set(pd.to_datetime(self.ds["time"].values))
        df = df[df["ts"].isin(valid_ts)].reset_index(drop=True)

        # 6) 去重：同一风暴同一时刻只保留一条（优先 rsize 大的）
        sort_cols = ["track_id", "ts"]
        if "rsize" in df.columns:
            df = df.sort_values(sort_cols + ["rsize"], ascending=[True, True, False])
        else:
            df = df.sort_values(sort_cols, ascending=[True, True])
        df = df.drop_duplicates(subset=["track_id", "ts"], keep="first").reset_index(drop=True)

        # （可选）同一天合并：关闭以保留 12h 间隔
        # 第 6 步已经完成了 (track_id + ts) 去重，这里不需要再按天合并
        merge_same_day = False
        if merge_same_day:
            df["date"] = df["ts"].dt.date
            if "rsize" in df.columns:
                df = df.sort_values(["track_id", "date", "rsize"], ascending=[True, True, False])
            else:
                df = df.sort_values(["track_id", "date"], ascending=[True, True])
            df = df.drop_duplicates(subset=["track_id", "date"], keep="first").reset_index(drop=True)

        # 7) 从 ts 拆回年/月/日/时（之后分组用）
        ...
        self.tracks = df.reset_index(drop=True)

        # 8) 分组（把 rsize 一并带入）
        cols = ["track_id", "lon", "lat"]
        if "rsize" in self.tracks.columns:
            cols.append("rsize")

        self.grouped_tracks = (
            self.tracks
            .drop_duplicates(subset=["track_id", "ts"])  # 再保险
            .groupby(["year", "month", "day", "hour"])
            .apply(lambda x: x[cols].values.tolist())
            .to_dict()
        )
    ...
    def __getitem__(self, idx):
        ...
        centers_with_r = []
        for rec in typhoon_positions:
            # rec 形如 [track_id, lon, lat] 或 [track_id, lon, lat, rsize]
            if len(rec) == 3:
                _, orig_lon, orig_lat = rec
                rsize_deg = self.label_radius  # 没有 rsize 列时，退回默认半径
            else:
                _, orig_lon, orig_lat, rsize_deg = rec
                if pd.isna(rsize_deg):
                    rsize_deg = self.label_radius


            # lon/lat → 1°网格索引（经度 0..359；纬度 +90 → 0..180）
            i_small = int(orig_lon)          # 经度
            j_small = int(orig_lat + 90)     # 纬度

            # 边界裁切（注意：height=181 ⇒ 索引 0..180）
            i_small = max(0, min(359, i_small))
            j_small = max(0, min(180, j_small))

            center_row_id = j_small
            center_col_id = i_small
            centers_with_r.append((center_row_id, center_col_id, rsize_deg))

        one_hot = data_merge_make_circular_one_hot_varradius_cpu(
            batch_size=batch,
            height=181,
            width=360,
            centers_with_radii=centers_with_r,
        )
