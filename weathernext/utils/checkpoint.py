...
def _convert_types(typ: type[_T], value: Any) -> _T:
  ...
  if dataclasses.is_dataclass(typ):
    ...
    return typ(**kwargs)  # pyrefly: ignore[bad-return]

  base_type = getattr(typ, "__origin__", None)

  if base_type is dict:
    assert len(typ.__args__) == 2  # pyrefly: ignore[missing-attribute]
    key_type, value_type = typ.__args__  # pyrefly: ignore[not-iterable]
    return {_convert_types(key_type, k): _convert_types(value_type, v)  # pyrefly: ignore[bad-return]
            for k, v in value.items()}

  if base_type is list:
    assert len(typ.__args__) == 1  # pyrefly: ignore[missing-attribute]
    value_type = typ.__args__[0]  # pyrefly: ignore[bad-index]
    return [_convert_types(value_type, v)  # pyrefly: ignore[bad-return]
            for _, v in sorted(value.items(), key=lambda x: int(x[0]))]

  if base_type is tuple:
    if len(typ.__args__) == 2 and typ.__args__[1] == ...:  # pyrefly: ignore[bad-index, missing-attribute]
      # An arbitrary length tuple of a single type, eg: tuple[int, ...]
      value_type = typ.__args__[0]  # pyrefly: ignore[bad-index]
      return tuple(_convert_types(value_type, v)  # pyrefly: ignore[bad-return]
                   for _, v in sorted(value.items(), key=lambda x: int(x[0])))
    else:
      # A fixed length tuple of arbitrary types, eg: tuple[int, str, float]
      assert len(typ.__args__) == len(value)  # pyrefly: ignore[missing-attribute]
      return tuple(  # pyrefly: ignore[bad-return]
          _convert_types(t, v)
          for t, (_, v) in zip(
              typ.__args__, sorted(value.items(), key=lambda x: int(x[0]))))  # pyrefly: ignore[bad-argument-type]

  # This is probably unreachable with reasonable serializable inputs.
  try:
    return typ(value)  # pyrefly: ignore[bad-argument-count]
