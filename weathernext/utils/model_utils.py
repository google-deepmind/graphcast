...
"""Utilities for building features for models."""
...
def lat_lon_to_leading_axes(
    grid_xarray: xr.DataArray) -> xr.DataArray:
...
def restore_leading_axes(grid_xarray: xr.DataArray) -> xr.DataArray:
...
def variable_to_stacked(
  ...
  """Converts an xr.Variable to preserved_dims + ("channels",).
  ...
  Returns:
    An xr.Variable with dimensions preserved_dims + ("channels",).
  """
...
def dataset_to_stacked(
  ...
  """Converts an xr.Dataset to a single stacked array.
  ...
  Returns:
    An xr.DataArray with dimensions preserved_dims + ("channels",).
    Existing coordinates for preserved_dims axes will be preserved, however
    there will be no coordinates for "channels".
  """
  data_vars = [
      variable_to_stacked(dataset.variables[name], sizes or dataset.sizes,  # pyrefly: ignore[bad-argument-type]
                          preserved_dims)
...
def stacked_to_dataset(
  ...
  Returns:
    An xr.Dataset (or other mapping of DataArrays) with the same shape and
    type as template_dataset.
  """
  unstack_from_channels_sizes = {}
  var_names = sorted(template_dataset.keys())  # pyrefly: ignore[bad-specialization]
...
DataArrayOrArray = TypeVar("DataArrayOrArray", xr.DataArray, np.ndarray)


def get_spatial_features(
    lat: DataArrayOrArray,
    lon: DataArrayOrArray,
    add_cartesian_positions: bool = False,
    add_sin_latitude: bool = False,
    add_sin_longitude: bool = False,
    add_cos_longitude: bool = False,
) -> dict[str, DataArrayOrArray]:
  """Returns an array of spatial features.

  `lat` and `lon` must be broadcastable against each other.

  Args:
    lat: The latitude of the points.
    lon: The longitude of the points.
    add_cartesian_positions: Whether to add the cartesian positions of the
      points (unit-sphere).
    add_sin_latitude: Whether to add the sine of the latitude.
    add_sin_longitude: Whether to add the sine of the longitude.
    add_cos_longitude: Whether to add the cosine of the longitude.

  Returns:
    A dict of arrays of spatial features.
  """

  # Use an ordered dict to ensure deterministic iteration order.
  features = collections.OrderedDict({})
  if add_cartesian_positions:
    (features["x"], features["y"], features["z"]
     ) = lat_lon_to_cartesian(lat, lon)  # pytype: disable=wrong-arg-types

  if add_sin_latitude:
    features["sin_lat"] = xu.sin(xu.deg2rad(lat))

  if add_sin_longitude:
    features["sin_lon"] = xu.sin(xu.deg2rad(lon))

  if add_cos_longitude:
    features["cos_lon"] = xu.cos(xu.deg2rad(lon))
...
def get_edge_features(
    ...
    add_scalar_distances: bool,
    add_vector_distances: bool,
    relative_latitude_vectors: bool,
    relative_longitude_vectors: bool,
    edge_normalization_factor: float | None = None,
    np_: NumpyInterface = np,
    transform_: TransformInterface = transform,
) -> dict[str, np.ndarray]:
  """Returns an array of edge features.

  `sender_lat`, `sender_lon`, `receiver_lat`, `receiver_lon`, needs to be
  broadcastable against each other across all dimensions by the first one.

  Args:
    sender_lat: The latitude of the sender points.
    sender_lon: The longitude of the sender points.
    receiver_lat: The latitude of the receiver points.
    receiver_lon: The longitude of the receiver points.
    sender_indices: The indices of edges to the sender points.
    receiver_indices: The indices of edges to the receiver points.
    add_scalar_distances: Whether to add the scalar distances between the sender
      and receiver points.
    add_vector_distances: Whether to add the vector distances between the sender
      and receiver points.
    relative_latitude_vectors: See
      `get_bipartite_relative_position_in_receiver_local_coordinates`
    relative_longitude_vectors:  See
      `get_bipartite_relative_position_in_receiver_local_coordinates`
    edge_normalization_factor: Optional constant to normalize the distance
      vectors. If None, the maximum distance across all edges is used.
    np_: Numpy library interface.
    transform_: scipy.transform library interface.


  Returns:
    A dict of arrays of edge features.
  """

  (senders_node_phi, senders_node_theta) = (
      lat_lon_deg_to_spherical(sender_lat, sender_lon, np_=np_)
  )
  (receivers_node_phi, receivers_node_theta) = (
      lat_lon_deg_to_spherical(receiver_lat, receiver_lon, np_=np_)
  )
  ...
  relative_position = (
      get_bipartite_relative_position_in_receiver_local_coordinates(
          ...
          receivers_node_phi=receivers_node_phi,
          receivers_node_theta=receivers_node_theta,
          senders=sender_indices,
          receivers=receiver_indices,
          latitude_local_coordinates=relative_latitude_vectors,
          longitude_local_coordinates=relative_longitude_vectors,
          np_=np_,
          transform_=transform_,
      )
  )
  ...
  # Note this is L2 distance in 3d space, rather than geodesic distance.
  relative_edge_distances = np_.linalg.norm(
      relative_position, axis=-1, keepdims=True
  )
  ...
  relative_edge_distances = relative_edge_distances / edge_normalization_factor
  relative_position = relative_position / edge_normalization_factor

  # Use an ordered dict to ensure deterministic iteration order.
  edge_features = collections.OrderedDict({})
  if add_scalar_distances:
    edge_features["distance"] = relative_edge_distances.squeeze(-1)

  if add_vector_distances:
    edge_features["rel_x"] = relative_position[..., 0]
    edge_features["rel_y"] = relative_position[..., 1]
    edge_features["rel_z"] = relative_position[..., 2]
