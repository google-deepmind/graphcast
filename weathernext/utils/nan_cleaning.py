...
class NaNCleaner(base.Predictor):
  ...
  def _maybe_reintroduce_nans(
    ...
    if self._var_to_clean in predictions.keys():
      nan_mask = xu.isnan(stale_inputs[self._var_to_clean]).any(dim='time')
  ...
  def __call__(
    ...
    predictions = self._predictor(
        inputs, targets_template, forcings, **kwargs  # pyrefly: ignore[bad-argument-type]
    )
    if self._reintroduce_nans:
      predictions = self._maybe_reintroduce_nans(original_inputs, predictions)  # pyrefly: ignore[unbound-name]
    return predictions
  ...
  def loss(
    ...
    return self._predictor.loss(
        inputs, targets, forcings, **kwargs  # pyrefly: ignore[bad-argument-type]
    )
