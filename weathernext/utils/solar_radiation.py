...
_DEFAULT_TSI_DATA_LOADER: TsiDataLoader = era5_tsi_data  # pylint: disable=invalid-name
...
def get_tsi(
  ...
  """
  timestamps = pd.DatetimeIndex(timestamps)  # pyrefly: ignore[bad-assignment]
  timestamps_date = pd.DatetimeIndex(timestamps.date)  # pyrefly: ignore[missing-attribute]
  day_fraction = (timestamps - timestamps_date) / pd.Timedelta(days=1)
  year_length = 365 + timestamps.is_leap_year  # pyrefly: ignore[missing-attribute]
  year_fraction = (timestamps.dayofyear - 1 + day_fraction) / year_length  # pyrefly: ignore[missing-attribute]
  fractional_year = timestamps.year + year_fraction  # pyrefly: ignore[missing-attribute]
...
def _get_orbital_parameters(j2000_days: chex.Array) -> _OrbitalParameters:
  ...
  rotational_phase = j2000_days % 1.0  # pyrefly: ignore[unsupported-operation]
  ...
  return _OrbitalParameters(
      theta=theta,
      rotational_phase=rotational_phase,  # pyrefly: ignore[bad-argument-type]
  ...
  for idx, timestamp in enumerate(timestamps):
    results.append(
        fn(
            ...
            longitude=lon,
            tsi=tsi[idx],  # pyrefly: ignore[bad-index]
...
def get_toa_incident_solar_radiation_for_xarray(
  ...
  if "time" in data_array_like.dims:
    ...
    output = xa.DataArray(radiation[0], dims=("lat", "lon"))  # pyrefly: ignore[bad-index]
