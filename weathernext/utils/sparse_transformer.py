...
from weathernext.utils import sparse_transformer_utils as utils
import haiku as hk
import jax
...
class _ModelConfig:
  ...
  # Upcast attention Q, K, V to float32 before splash attention.
  upcast_attn_to_fp32: bool = False
...
def triblockdiag_mha(q_input: jnp.ndarray, kv_input: jnp.ndarray,
  ...
  logits_d = qk_prod(q, k[:, 1:-1, ...]) * cfg.key_size**-0.5  # pyrefly: ignore[unsupported-operation]
  logits_u = qk_prod(q, k[:, 2:, ...]) * cfg.key_size**-0.5  # pyrefly: ignore[unsupported-operation]
  logits_l = qk_prod(q, k[:, :-2, ...]) * cfg.key_size**-0.5  # pyrefly: ignore[unsupported-operation]
  ...
  logits_d, logits_u, logits_l = utils.wrap_fn_for_upcast_downcast(
      ...
      triblockdiag_softmax  # pyrefly: ignore[bad-argument-type]
      )
  ...
  x = jnp.reshape(x, x.shape[:-2] + (cfg.num_heads * cfg.value_size,))  # pyrefly: ignore[unsupported-operation]
...
def multihead_linear(
  ...
  attn_winit = hk.initializers.VarianceScaling(cfg.attn_winit_mult /
                                               cfg.num_layers)
  out = hk.Linear(
      cfg.num_heads * head_size,  # pyrefly: ignore[unsupported-operation]
      w_init=attn_winit,
      name='mha_proj_' + qkv,
      with_bias=False,
  )(x)
...
def mha(q_input: jnp.ndarray, kv_input: jnp.ndarray,
  ...
  if normalize_logits:
    logits *= cfg.key_size**-0.5  # pyrefly: ignore[unsupported-operation]
  if mask is not None:
    def apply_mask(m, l):
  ...
  weights = utils.wrap_fn_for_upcast_downcast(logits, jax.nn.softmax)  # pyrefly: ignore[bad-argument-type]
...
def _make_splash_mha(
  ...
  block_sizes = splash_attention.BlockSizes(
      block_q=block_q,  # pyrefly: ignore[bad-argument-type]
      block_kv=block_kv,  # pyrefly: ignore[bad-argument-type]
...
def splash_mha(q_input: jnp.ndarray, kv_input: jnp.ndarray,
  ...
  if q_padding > 0:
    q_input = jnp.pad(q_input, ((0, 0), (0, q_padding), (0, 0)))

  if kv_padding > 0:
    kv_input = jnp.pad(kv_input, ((0, 0), (0, kv_padding), (0, 0)))
  ...
  attn = _make_splash_mha(
      mask=mask,
      mask_type=cfg.mask_type,  # pyrefly: ignore[bad-argument-type]
      num_heads=num_heads,
      block_q=cfg.block_q,
...
def mask_block_diags(mask: sp.sparse.csr_matrix,
  ...
  mask_lower_diag_blocks = jnp.stack(
      ...
      + [jnp.array(mask[(i + 1) * block_size : (i + 2) * block_size,
         ...
         for i in range(mask.shape[0] // block_size - 1)])
  mask = jnp.stack(  # pyrefly: ignore[bad-assignment]
      [mask_daig_blocks, mask_upper_diag_blocks, mask_lower_diag_blocks]
  )
  mask = jnp.expand_dims(mask, (0, 3))  # pyrefly: ignore[bad-argument-type, bad-assignment]
...
class WeatherMeshMask(splash_attention.splash_attention_mask.Mask):
  ...
  def __getitem__(self, idx) -> np.ndarray:
    ...
    return self.mask[q_slice, kv_slice].toarray()  # pyrefly: ignore[bad-index]
  ...
  def __call__(self, x, global_norm_conditioning=jax.Array):
    ...
    def attn(x):
      if self._cfg.attention_type == 'triblockdiag_mha':
        ...
        return splash_mha(x, x, mask=self.mask, cfg=self._cfg,
                          q_padding=self.num_padding_nodes[0],
                          kv_padding=self.num_padding_nodes[1])
      else:
        raise NotImplementedError()

    def norm_conditioning_layer(x):
      return dense.LinearNormConditioning(
          name=self.name+'_norm_conditioning')(
              x,
              norm_conditioning=jnp.expand_dims(global_norm_conditioning, 1)  # pyrefly: ignore[bad-argument-type]
              )
...
class Transformer(hk.Module):
  ...
  def __init__(self,
    ...
    if attention_type == 'triblockdiag_mha':
      ...
      max_q_block_size = np.maximum(block_q, block_q_dkv)  # pyrefly: ignore[no-matching-overload]
      max_kv_block_size = np.maximum(block_kv, block_kv_dkv)  # pyrefly: ignore[no-matching-overload]
