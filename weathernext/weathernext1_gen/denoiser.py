...
from weathernext.utils.legacy import deep_typed_graph_net
from weathernext.utils.legacy import grid_mesh_connectivity
from weathernext.weathernext1_gen import denoisers_base as base
from weathernext.weathernext1_gen import transformer
...
class Denoiser(base.Denoiser):
  ...
  def __init__(
    ...
    self._noise_level_encoder = FourierFeaturesMLP(**noise_encoder_config)  # pyrefly: ignore[bad-unpacking]
...
class _DenoiserArchitecture:
  ...
  * Mesh2Grid graph: Graph that contains all nodes. This graph is strictly
  ...
  def _init_grid2mesh_graph(self) -> typed_graph.TypedGraph:
    ...
    (senders_node_features, receivers_node_features,
     edge_features) = model_utils.get_bipartite_graph_spatial_features(
         senders_node_lat=self._grid_nodes_lat,  # pyrefly: ignore[bad-argument-type]
         senders_node_lon=self._grid_nodes_lon,  # pyrefly: ignore[bad-argument-type]
         receivers_node_lat=self._mesh_nodes_lat,  # pyrefly: ignore[bad-argument-type]
         receivers_node_lon=self._mesh_nodes_lon,  # pyrefly: ignore[bad-argument-type]
         senders=senders,
         receivers=receivers,
  ...
  def _init_mesh2grid_graph(self) -> typed_graph.TypedGraph:
    ...
    (grid_indices,
     mesh_indices) = grid_mesh_connectivity.in_mesh_triangle_indices(
         grid_latitude=self._grid_lat,  # pyrefly: ignore[bad-argument-type]
         grid_longitude=self._grid_lon,  # pyrefly: ignore[bad-argument-type]
         mesh=self._mesh)
    ...
    (senders_node_features, receivers_node_features,
     edge_features) = model_utils.get_bipartite_graph_spatial_features(
         ...
         receivers_node_lat=self._grid_nodes_lat,  # pyrefly: ignore[bad-argument-type]
         receivers_node_lon=self._grid_nodes_lon,  # pyrefly: ignore[bad-argument-type]
         senders=senders,
         receivers=receivers,
         edge_normalization_factor=None,
         **self._spatial_features_kwargs,
  ...
  def _run_grid2mesh_gnn(self, grid_node_features: chex.Array,
    ...
    input_graph = self._grid2mesh_graph_structure._replace(  # pyrefly: ignore[missing-attribute]
...
def _permute_mesh_to_banded(mesh):
  ...
  # Note `get_permutation_to_banded` calls `faces_to_edges` internally.
  # so to make sure ordering is preserved, any changes to faces_to_edges should
  # should be reflected in the other 2 calls to faces_to_edges in this file.
  mesh_permutation, permute_func = icosahedral_mesh.get_permutation_to_banded(
      mesh)
