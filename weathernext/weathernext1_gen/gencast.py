...
from weathernext.weathernext1_gen import dpm_solver_plus_plus_2s
from weathernext.weathernext1_gen import samplers_utils
import haiku as hk
import jax
import xarray
import xarray_jax
...
TASK = task_base.TaskConfig(
    ...
    forcing_variables=variables.TIME_FORCING_VARS,
...
class CheckPoint:
  ...
  license: str
  params: dict[str, Any]
  task_config: task_base.TaskConfig
...
class GenCast(predictor_base.Predictor):
  ...
  def __init__(
    ...
    num_surface_vars = len(
        ...
        - set(variables.ALL_ATMOSPHERIC_VARS)
    )
  ...
  def loss_and_predictions(  # pyrefly: ignore[bad-override]
      self,
      inputs: xarray.Dataset,
  ...
  def loss(self,  # pyrefly: ignore[bad-override]
           inputs: xarray.Dataset,
           targets: xarray.Dataset,
           forcings: Optional[xarray.Dataset] = None,
  ...
  def __call__(self,
    ...
    if self._sampler is None:
      self._sampler = dpm_solver_plus_plus_2s.Sampler(  # pyrefly: ignore[missing-argument]
          self._preconditioned_denoiser, **self._sampler_config  # pyrefly: ignore[bad-unpacking]
      )
